...
/ip hotspot profile
add name="default" html-directory=flash/hotspot login-by=cookie,http-chap \
    http-cookie-lifetime=1d rate-limit="" smtp-server=0.0.0.0

/ip hotspot
add name="hotspot1" interface={{interface}} address-pool={{pool}} \
    profile=default disabled=no

# Hotspot User Profiles
/ip hotspot user profile
add name="1-hour" session-timeout=1h shared-users=1 rate-limit="2M/2M"
add name="1-day" session-timeout=1d shared-users=1 rate-limit="5M/5M"
add name="1-week" session-timeout=1w shared-users=1 rate-limit="10M/10M"
add name="1-month" session-timeout=4w2d shared-users=1 rate-limit="20M/20M"
...
/ip firewall nat
add chain=srcnat src-address={{network}} action=masquerade
EOF

    cat << 'EOF' > "$APP_DIR/src/mikrotik/templates/vpn-server-setup.rsc"
# MikroTik VPN Server Setup Template
# L2TP/IPSec Configuration

# Create IP Pool for VPN
/ip pool
add name=vpn-pool ranges={{vpn_pool}}

# Create PPP Profile
/ppp profile
add name=vpn-profile local-address={{local_address}} \
    remote-address=vpn-pool dns-server={{dns_servers}} \
    change-tcp-mss=yes use-encryption=yes

# Enable L2TP Server
/interface l2tp-server server
set enabled=yes default-profile=vpn-profile \
    authentication=mschap2 use-ipsec=yes \
...
add chain=input protocol=udp dst-port=1701 ipsec-policy=in,ipsec \
    action=accept comment="Allow L2TP"
EOF

    cat << 'EOF' > "$APP_DIR/src/mikrotik/templates/firewall-basic.rsc"
# MikroTik Basic Firewall Template
...
add chain=input protocol=icmp action=accept \
    comment="Accept ICMP"
add chain=input in-interface={{wan_interface}} action=drop \
...
add chain=forward connection-state=invalid action=drop \
    comment="Drop invalid"
add chain=forward in-interface={{lan_interface}} action=accept \
    comment="Accept from LAN"
add chain=forward in-interface={{wan_interface}} action=drop \
...
EOF
    ...
    cat << 'EOF' > "$APP_DIR/src/mikrotik/templates/qos-basic.rsc"
# MikroTik QoS Template
# Basic bandwidth management

# Mangle Rules for marking
/ip firewall mangle
add chain=forward protocol=tcp dst-port=80,443 action=mark-packet \
    new-packet-mark=http passthrough=yes comment="Mark HTTP/HTTPS"
add chain=forward protocol=tcp dst-port=25,110,143,465,587,993,995 \
    action=mark-packet new-packet-mark=email passthrough=yes comment="Mark Email"
add chain=forward protocol=udp dst-port=53 action=mark-packet \
    new-packet-mark=dns passthrough=yes comment="Mark DNS"

# Queue Tree
/queue tree
add name="Download" parent=global max-limit={{download_limit}}
add name="Upload" parent=global max-limit={{upload_limit}}

add name="HTTP-Download" parent=Download packet-mark=http \
    limit-at={{http_guaranteed}} max-limit={{http_max}} priority=4
add name="Email-Download" parent=Download packet-mark=email \
    limit-at={{email_guaranteed}} max-limit={{email_max}} priority=3
add name="DNS-Download" parent=Download packet-mark=dns \
    limit-at=256k max-limit=1M priority=1
EOF
...
# Create device routes
create_device_routes() {
    cat << 'EOF' > "$APP_DIR/routes/deviceRoutes.js"
...
const validateDevice = [
    ...
    body('macAddress').trim().notEmpty().matches(/^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/)
        .withMessage('Invalid MAC address format'),
    body('vpnIpAddress').isIP().withMessage('Invalid IP address'),
    body('apiUsername').trim().notEmpty().withMessage('API username is required'),
    body('apiPassword').trim().notEmpty().withMessage('API password is required')
];
...
// Register new device (admin/operator only)
router.post('/', 
...
// Discover devices on network (admin/operator only)
router.post('/discover', 
    authorize('admin', 'operator'),
    deviceController.discoverDevices
);

// Execute command on device (admin/operator only)
...
// Backup device configuration (admin/operator only)
router.post('/:id/backup', 
    authorize('admin', 'operator'),
    validateDeviceId,
    deviceController.backupDevice
);

// Reboot device (admin only)
...
const logDir = '/var/log/mikrotik-vpn';
...
EOF
        log "Logger utility created"
    fi
}

# Create notification utility if not exists
create_notification_utility() {
    if [[ ! -f "$APP_DIR/utils/notifications.js" ]]; then
        mkdir -p "$APP_DIR/utils"
        cat << 'EOF' > "$APP_DIR/utils/notifications.js"
...
async function sendAlert(alert) {
    ...
    if (alert.type === NotificationType.CRITICAL && process.env.SMS_API_KEY) {
        channels.push(sendSMSAlert(alert));
    }
...
async function sendEmailAlert(alert) {
    ...
    const mailOptions = {
        from: process.env.SMTP_FROM || 'MikroTik VPN <noreply@mikrotik-vpn.local>',
        to: process.env.ALERT_EMAIL_TO,
        subject: `[${alert.type.toUpperCase()}] ${alert.message}`,
        html: `
            <h2>MikroTik VPN Alert</h2>
            <p><strong>Type:</strong> ${alert.type}</p>
            <p><strong>Device:</strong> ${alert.device || 'N/A'}</p>
...
async function sendSlackAlert(alert) {
    ...
    await webhook.send({
        attachments: [{
            ...
            footer: 'MikroTik VPN Management System'
        }]
    });
}
...
async function sendSMSAlert(alert) {
    ...
    logger.info('SMS alert would be sent', { alert });
}
...
EOF
        log "Notification utility created"
    fi
}

# Main execution
main() {
    log "Starting Phase 2.1: MikroTik Device Management Setup"
    ...
    log "Phase 2.1 completed successfully!"
    log "You can now proceed with Phase 2 Part 2"
}
